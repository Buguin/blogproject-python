...
class Post(models.Model):
    ...
    # 文章正文，我们使用了 TextField。比较短的字符串存储可以使用 CharField，但对于文章的正文来说可能会是一大段文本，因此使用 TextField 来存储大段文本。
    body = models.TextField()

    # 创建时间与修改时间，这两个列分表表示了文章的创建时间和最后一次修改时间，存储时间的列用 DateTimeField。
    ...
    # 文章作者，这里 User 是从 django.contrib.auth.models 导入的。django.contrib.auth 是 django 内置的应用，专门用于处理网站用户的注册、登录等流程，User 是 django 为我们已经写好的用户模型，这里我们通过 ForeignKey 把文章和 User 关联起来，因为我们规定一篇文章只能有一个作者，而一个作者可能会写多篇文章，因此这是一对多的关系，和 Category 类似。
